# 2026-04-10 - BluStellar リテール向けシナリオ/オファリング Deep Dive

## 背景
自社入社（2026-06-01）を前に、BluStellarの全体構造を理解した上で、リテール（流通・小売）向けのシナリオ/オファリング体系を深掘りする。入社後に提案書を書く際の「共通言語」としてBluStellarを使いこなすことが目的。

→ 前回調査: [自社中期経営計画・関連ニュース調査レポート](2026-04-09-midterm-plan-research.md)
→ 関連Deep Dive: [cotomi & Agentic AI](2026-04-10-cotomi-agentic-ai-deep-dive.md) / [財務ブレイクダウン](2026-04-10-midterm-plan-financial-breakdown.md)
...
CDO吉崎氏: *「オファリングをシナリオに集約した。価値は自社が決めるのではなく、顧客が決める」*
...
## 3. 自社 リテールDXの全体コンセプト: "Smart Retail CX"

自社は流通・小売向けDXを **"Smart Retail CX"** として体系化。3つの柱がある:
...
| **自社 映像分析基盤 (EVA)** | カメラ入力・映像処理の共通基盤 + 複数分析ユニットを柔軟に組合せ | Products & Services | AI系 / CX |
...
| **自社 棚定点観測サービス** | 固定カメラ映像からAIが棚の陳列状況を可視化。イオンリテール、東急ストアが先行採用 | Products & Services → Offerings | 業種別(流通) |
| **Digital Store Platform** | Smart Retail CXの情報システム基盤。データ透明性、リアルタイム対応 | Offerings | 業種別(流通) |
| **映像認識AI + LLM**（主要クライアント実証） | 従業員の作業映像をAI分類 → LLMがレポート自動作成 | Products & Services(先端) | AI系 / 業種別 |

**重要ポイント**: 主要クライアント案件で使われている映像認識AI + LLMは、BluStellarの **AI系シナリオ × 業種別(流通)シナリオの交差点** に位置する。

---

## 5. BluStellar リテール事例

### 事例A: 競合小売チェーン 次世代店舗システム（最重要・最大規模）

- **規模**: 全国21,000店舗、約30万台のモバイル端末/タブレット、約40万人の従業員
- **推定規模**: 500億円・3,000人体制
- **技術**: 国内コンビニ初のフルクラウド型業務システム（Google Cloud基盤）+ マイクロサービスアーキテクチャ + 自社顔認証 + ServiceNow ITSM + Omnissa Workspace ONE
- **BluStellar連携**: 公式に「BluStellarのアプローチに基づき、構想立案から運用までサポート」と明記
- **意義**: シナリオチェーン型（コンサル→構築→運用の一気通貫）の最大成功事例

### 事例B: 主要クライアント × 自社 映像認識AI実証（配属先直結）
...
- **目標**: 2回目購入率の10%向上
- **意義**: リテール業種別CXシナリオの公開事例。主要クライアントのPontaデータ活用にも応用可能なパターン

### その他

- 主要クライアント × ターゲティング広告AI: 商品購入率**約12倍**に向上
...
| グループ展開 | 自社本体 | NESIC/ネクサ/フィールディングに展開開始 |
...
→ 詳細は [cotomi & Agentic AI Deep Dive](2026-04-10-cotomi-agentic-ai-deep-dive.md) 参照
...
| パターン | 説明 | 主要クライアントでの具体例 |
|---|---|---|
| **シナリオチェーン** | コンサル→構築→運用を一気通貫 | 映像AI分析コンサル → システム構築 → 運用保守契約 |
| **シナリオ横展開** | 成功した型を別クライアントへ | 競合小売案件の知見 → 主要クライアントへ横展開 |
| **シナリオ発展** | 1つの施策から隣接テーマへ | 映像AI作業分析 → 客動線分析 → 棚割り最適化 → OMO |

### 自社プロダクト比率の強化方針

現在BluStellarの約40%がサードパーティ製品。次期中計で **自社自社プロダクト比率を引き上げる** 方針。

→ 提案書で自社製品（NeoSarf、顔認証、cotomi、EVAなど）を組み込むインセンティブが強まる。
...
| 自社プラットフォームズ（製造・SCM） | Industrial IoT Platform を国内4拠点に導入 | 初年度 **2%損益改善**、2034年まで **420% ROI** |
| 食品・飲料マーケティング | AIでコピー＆動画コンテンツ生成 | 動画視聴数 **125%増** |

---

## 9. 操縦者向け: 実践的インプリケーション

### 映像認識AI弾込め（別ファイル）との接続

[映像認識AI × 売上向上 弾込めリサーチ](2026-04-09-video-ai-sales-uplift-ammo.md) で策定した3層提案をBluStellar語に翻訳すると:
...
### 入社後に使えるBluStellar語の翻訳フレーズ

| こう言いたい時 | BluStellar語 |
|---|---|
| 「映像AIで店舗業務を効率化したい」 | 「AI系シナリオで、EVAと映像認識AIのオファリングを組み合わせた店舗作業DXの提案」 |
| 「顧客の動線を分析して売上を上げたい」 | 「CXシナリオで、人物行動分析とヒートマップのオファリングによる顧客体験変革」 |
| 「競合小売でやったことを主要クライアントでもやりたい」 | 「競合小売案件のシナリオ横展開。主要クライアント固有の経営課題に合わせたシナリオカスタマイズ」 |
| 「小さく始めて大きく育てたい」 | 「シナリオ発展型のアプローチ。Layer 1で実績を作り、隣接シナリオへ拡張」 |

### 競合小売案件との関係性

競合小売次世代店舗システムはBluStellarリテールシナリオの**最大の成功事例**。主要クライアント向け提案の「型」の基盤になるが、**そのまま横展開ではなく主要クライアント固有の経営課題に合わせたシナリオ化**が求められる。
...
### リテールDX
- [自社 リテールDX(Smart Retail CX)公式](https://jpn.nec.com/retail/index.html)
- [NeoSarf/POS 公式](https://jpn.nec.com/neosarf/pos/index.html)
- [storeGATE/L](https://www.nec-nexs.com/sl/retail/storegate_l.html)
- [自社 映像分析基盤 EVA](https://jpn.nec.com/video-analytics/eva/index.html)
- [自社 棚定点観測サービス](https://jpn.nec.com/press/202403/20240311_02.html)
...
### 事例
- [競合小売チェーン次世代店舗システム](https://jpn.nec.com/press/202505/20250522_01.html)
- [主要クライアント×自社 映像認識AI実証](https://jpn.nec.com/press/202510/20251030_02.html)
- [BluStellar Scenario事例(ダイアナ含む)](https://cloud.watch.impress.co.jp/docs/news/2087118.html)
- [自社 流通DX事例一覧](https://jpn.nec.com/retail/work/index.html)
...
### クライアントゼロ
- [自社のCIOが語るクライアントゼロ企業変革](https://www.cio.com/article/4037782/)
...
- [自社プラットフォームズ IoT導入事例](https://jpn.nec.com/manufacture/monozukuri/iot/case/necplatforms/index.html)

## アクションアイテム
- [ ] 自分 / BluStellar Scenario公式ページで32シナリオの一覧を確認 / 2026-05-15
- [ ] 自分 / 自社 リテールDX(Smart Retail CX)公式を精読 / 2026-05-15
- [ ] 自分 / 競合小売次世代店舗システムの公開情報を全て読破 / 2026-05-20
- [ ] 自分 / ダイアナ事例（CXシナリオ適用）の詳細を確認 / 2026-05-25
- [ ] 自分 / 弾込め3層提案のBluStellar語翻訳を暗記 / 2026-05-31
